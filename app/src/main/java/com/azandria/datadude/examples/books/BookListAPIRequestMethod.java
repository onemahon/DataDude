...
public class BookListAPIRequestMethod implements IDataRequestMethod<List<Book>> {

    private String mSearchString;

    public BookListAPIRequestMethod(String searchString) {
    ...
    public void doRequest(final DataRequestBuilder.RequestMethodListener<List<Book>> listener, Collection<IDataRequestFilter> filters) {
        BasicWebServiceManager.get().doRequestInBackground(new BookListApiRequest(mSearchString), new WebServiceRequestListener<List<Book>>() {
            @Override
            public void onRequestComplete(WebServiceManager webServiceManager, ResultInfo<List<Book>> resultInfo) {
                if (resultInfo.isStatusOK()) {
                    ...
                    BookListManager.get().store(mSearchString, resultInfo.getResult());
                } else {
